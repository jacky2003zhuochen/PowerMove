...
def continuous_router(mg, pos, empty_space, move_out_qubits, move_in_qubits, storage_flag, Row, location_index, target_location_index, location_size, method, h, a1):
    ...
    init_space = {k: v[:] for k, v in empty_space.items()}
    move_out_set = set(move_out_qubits)
    # print("pos", pos)
    # print("empty_space", empty_space)
    # print("move_out_qubits", move_out_qubits)
    # print("move_in_qubits", move_in_qubits)
    for move in mg:
        q0 = move[0]
        q1 = move[1]
        pos_q0 = pos[q0]
            ...
            for r in range(10* Row):
                for i in range(min(r + 1, 4*Row)):
                    ...
                    for a in [-1, 1]:
                        npos_x = pos_x + a * i
                        npos_y = pos_y - j
                        # if npos_x >= 0 and npos_x < 4*Row and npos_y < 0 and npos_y >= -math.ceil(Row/2) and len(empty_space[(npos_x, npos_y)]) == 0:
                        if npos_x >= 0 and npos_x < 4*Row and npos_y < 0 and npos_y >= -math.ceil(Row/(2*a1**2)) and len(empty_space[(npos_x, npos_y)]) == 0:
                            storage_in_move[q] = (npos_x, npos_y)
                            empty_space[(npos_x, npos_y)].append(q)
                            target_location_index[q] = 0
                            break
                    if q in storage_in_move:
                        break
                if q in storage_in_move:
                    break
    ...
    #             #     static_pos[p] = pq
    #             if pq not in moved_qubits and pq not in redundant_qubits_to_be_moved: # move all not 2-qubit gate related qubit
    ...
    for qm in qmg:
        if pos[qm[1]] in static_pos.keys() and qm[1] == static_pos[pos[qm[1]]]:
            ...
            target_location_index[qm[0]] = (location_index[qm[1]] + 1) % 2
            move_group.append((
                qm[0], 
    ...
    # print("empty_space2", empty_space)
